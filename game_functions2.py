...
def update_screen(ai_settings, screen, ship, aliens, bullets, constellation=None, rain=None):
...
def ship_hit(ai_settings, stats, screen, ship, aliens, bullets):
    """Respond to ship being hit by alien. Game over."""
...
def create_star(screen, constellation, star_number, row_number):
...
def create_constellation(ai_settings, screen, constellation):
    """Create a constellation of stars."""
    # Create an star, and find number of stars in a row.
    star = Star(screen)
    number_stars_x = get_number_stars_x(ai_settings, star.rect.width)
    number_rows = ai_settings.screen_height // star.rect.height
    
    # Create the grid of stars.
    for row_number in range(number_rows):
        for star_number in range(number_stars_x):
            create_star(screen, constellation, star_number,
                row_number)

def create_raindrop(screen, rain, drop_number, row_number):
    """Create an raindrop, and place it in the row."""
    raindrop = Raindrop(screen)
    raindrop_width = raindrop.rect.width
    raindrop.x = raindrop_width + 2 * raindrop_width * drop_number
    raindrop.rect.x = raindrop.x
    raindrop.rect.y = raindrop.rect.height + raindrop.rect.height * row_number
    rain.add(raindrop)
...
def create_rain(ai_settings, screen, rain):
    ...
    # Create a raindrop, and find number of raindrops in a row.
    raindrop = Raindrop(screen)
    number_raindrops_x = get_number_raindrops_x(ai_settings, raindrop.rect.width)
    number_rows = ai_settings.screen_height // raindrop.rect.height
    
    # Create the grid of drops.
    for row_number in range(number_rows):
        for drop_number in range(number_raindrops_x):
            create_raindrop(screen, rain, drop_number,  row_number)